...
| C-48 | **Kezbesitesi ertesito** | ⏳ **megepult — jovahagyasra var** (2026-09-07) | ⭐ **Owner-korrekcio utan:** a **kuldes** pillanataban megy egy tomondat (`📨 Átment 3 üzeneted.`). ⛔ A **varakozasrol** nem szolunk — azt a „gepel…" fedi (felso korlat 15 → 45 perc). ⛔ NEM szamit valasznak | `cli/src/discord/discord.receipt.ts` |
| C-46 | **Távvezérlés-szűrő a jelenlétben** | ⏳ **megépült — jóváhagyásra vár** (2026-09-07) | Felismeri a RustDesk-munkamenetet, és a távolról generált bevitelt NEM veszi fizikai jelenlétnek (`unknown`, nem `yes`) | `cli/src/presence/presence.remote-session.ts` |
| C-47 | **Telefon-helyzet lekérése** | 🔨 **ÉPÜL — a mag kész** (2026-09-07) | ✅ Owner-döntés: **OwnTracks**, állítható gyakoriság, ⛔ az otthoni koordinátát NEM tároljuk. A mag (feldolgozás + tárolási szabály) **kész, 21 teszt**. ⏳ Hátra: a HTTP-végpont + a telefon-oldali beállítás | `cli/src/location/` · `current/principles/location-retention.md` |
| C-45 | **Köteg-frissítés küldés előtt** | ⏳ **megépült — jóváhagyásra vár** (2026-09-07) | Ha a köteg még vár, kiküldés előtt lekérdezi a Discordtól az **aktuális** szöveget: a javított változat megy át, a törölt kiesik. ⛔ Lekérdezési hibánál a régi megy — hálózati hiba nem törölhet üzenetet | `cli/src/discord/discord.batch-refresh.ts` |
