...
describe('My Assistant Chrome companion background', (): void => {
  it('accepts only the exact loopback app origin', (): void => {
    assert.equal(isTrustedMyAssistantUrl('http://127.0.0.1:39335/linkedin'), true);
    assert.equal(isTrustedMyAssistantUrl('http://localhost:39335/dashboard'), true);
    assert.equal(isTrustedMyAssistantUrl('https://127.0.0.1:39335/linkedin'), false);
    assert.equal(isTrustedMyAssistantUrl('http://127.0.0.1.evil.test:39335/linkedin'), false);
  ...
  it('starts the panel and top-level LinkedIn tab in the same user gesture', async (): Promise<void> => {
    const calls: string[] = [];
  ...
  it('opens LinkedIn and returns a truthful panel diagnostic when the panel is unavailable', async (): Promise<void> => {
    let tabOpened: boolean = false;
    const response = await openLinkedInWorkspace({
      openSidePanel: async (): Promise<void> => { throw new Error('sidePanel unavailable'); },
      createTab: async (): Promise<void> => { tabOpened = true; },
    }, 7, 'request-2');
    assert.equal(response.ok, false);
    assert.equal(response.code, 'PANEL_UNAVAILABLE');
