...
function readVoiceFunnel(): PulseVoiceFunnel | undefined {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.voice-stats', err);
...
function readDiscordHeartbeat(now: Date): SystemPulseSnapshot['discord'] {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.discord-heartbeat', err);
...
function readNewestPresenceSample(now: Date): SystemPulseSnapshot['presence'] {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.presence-sample', err);
...
function readLastSample(file: string): { timestampMs: number; idleState?: string } | null {
  ...
  for (let i: number = lines.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      SwallowedFailure_Util.report(`system-pulse.presence-line:${file}`, err);
      continue;
    }
  }
...
function countSttRetryPending(): number {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.stt-retry-count', err);
...
function countPendingInbound(): number {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.pending-inbound-count', err);
...
function readLastOutboundAge(now: Date): number | undefined {
  try {
    ...
    SwallowedFailure_Util.report('system-pulse.last-outbound-age', err);
...
export class SystemPulse_Service {
  ...
  private tick(): void {
    try {
      ...
      SwallowedFailure_Util.report('system-pulse.pulse-tick', err);
    }
  }
}
